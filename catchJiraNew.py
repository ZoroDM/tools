...
excel_name = "buglist-logic-0104.csv"#excel文件名
csv_file = open(excel_name,"w",newline='')
writer = csv.writer(csv_file)
writer.writerow(['项目','主题','bug链接','模块','经办人','状态','严重程度','解决人','概率性故障','优先级','创建者','解决结果','关注人','bug打回次数','Developer','更新日期'])
...
#登录jira
driver.find_element_by_id('login-form-username').send_keys("yl1133")#账号
...
for link in link_list:
    ...
    for i in range(len(data)):#issuerow52811 > td.summary#issuerow52811 > td.summary > p > a
        ...
        #writer.writerow([project,summary,bug_url,components,assignee,created_time,status,bug_level,who_solve,gailv,priority,creator,resolution,watches,bugDahui,Developer,updated_time])
        writer.writerow([project,summary,bug_url,components,assignee,status,bug_level,who_solve,gailv,priority,creator,resolution,watches,bugDahui,Developer,updated_time])
    print('success')
csv_file.close()
